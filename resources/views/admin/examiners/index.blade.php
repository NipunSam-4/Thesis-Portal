<x-app-layout>
    ...
    <div class="py-4 sm:py-8" x-data="{ 
        ...
        <div class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 space-y-4 sm:space-y-6">
            ...
            <x-admin-header title="External Examiners" description="Manage external thesis evaluation panel members and active portal access." />

            <!-- Alerts -->
            @if(session('success'))
                <div class="mb-4 sm:mb-6 bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800 text-emerald-800 dark:text-emerald-300 px-4 py-3 rounded-xl text-sm flex items-center shadow-sm">
            ...
            <div class="bg-white dark:bg-slate-900 p-4 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800">
                <div class="relative w-full sm:w-96">
                    ...
                    <input type="text" x-model="searchQuery" placeholder="Search examiner by name or email" class="w-full pl-10 pr-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-indigo-500 focus:border-indigo-500 dark:text-white">
                </div>
            </div>

            <!-- Examiners Table -->
            <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
                <div class="p-4 sm:p-5 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50 flex justify-between items-center">
                    <div>
                        <h3 class="font-bold text-slate-800 dark:text-slate-200">External Examiners Panel</h3>
                        <span class="text-xs text-slate-500">Total Examiners: {{ $examiners->count() }}</span>
                    </div>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody x-ref="tableBody" class="divide-y divide-slate-100 dark:divide-slate-800 text-sm">
                            @forelse($examiners as $user)
                                @php
                                    $targetSearch = strtolower($user->name . ' ' . $user->email);
                                @endphp
                                <tr data-sortable="true" data-name="{{ addslashes($user->name) }}" data-email="{{ addslashes($user->email) }}" data-role="{{ $user->role }}" data-status="{{ $user->is_active ? 'active' : 'inactive' }}" x-show="matchesFilter('{{ addslashes($targetSearch) }}')" class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition">
                                    ...
                                    <td class="p-4 text-center min-w-[180px]">
                                        <div class="flex items-center justify-center space-x-3">
                                            ...
                                            @if($user->is_active)
                                                <button type="button" @click="openConfirm('Deactivate External Examiner', 'Are you sure you want to deactivate {{ addslashes($user->name) }}?', '{{ route('admin.users.toggle', $user->id) }}', 'Deactivate Examiner', 'bg-rose-600 hover:bg-rose-700 text-white')" class="text-rose-600 hover:text-rose-900 dark:text-rose-400 dark:hover:text-rose-300 font-medium px-3 py-1 rounded bg-rose-50 hover:bg-rose-100 dark:bg-rose-900/30 dark:hover:bg-rose-900/50 transition text-xs w-24 text-center">
                                                    Deactivate
                                                </button>
                                            @else
                                                <button type="button" @click="openConfirm('Reactivate External Examiner', 'Are you sure you want to reactivate {{ addslashes($user->name) }}?', '{{ route('admin.users.toggle', $user->id) }}', 'Reactivate Examiner', 'bg-emerald-600 hover:bg-emerald-700 text-white')" class="text-emerald-600 hover:text-emerald-900 dark:text-emerald-400 dark:hover:text-emerald-300 font-medium px-3 py-1 rounded bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-900/30 transition text-xs w-24 text-center">
                                                    Activate
                                                </button>
                                            @endif
                                        </div>
        ...
        <!-- Edit Examiner Name Modal (Name Only) -->
                ...
                <div x-show="editModalOpen" class="inline-block align-bottom bg-white dark:bg-slate-900 rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full border border-slate-200 dark:border-slate-800">
                    <form :action="`/admin/users/${editId}/name`" method="POST">
                        ...
                        <div class="bg-white dark:bg-slate-900 px-6 pt-6 pb-4">
                            <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-4">Edit Examiner Name</h3>
