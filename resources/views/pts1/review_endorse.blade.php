<x-app-layout>
    ...
    <div class="py-8" x-data="{
        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <!-- Section 2: Seminar & Confirmation Details (Read-Only) -->
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Date of Confirmation</label>
                        <input type="text" value="{{ $student->date_confirmation ? \Carbon\Carbon::parse($student->date_confirmation)->format('F d, Y') : 'N/A' }}" readonly class="w-full bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 font-semibold rounded-lg border-gray-300 dark:border-gray-600 cursor-not-allowed">
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Date of Open Seminar</label>
                        <input type="text" value="{{ $pts1->seminar_date ? $pts1->seminar_date->format('F d, Y') : 'N/A' }}" readonly class="w-full bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 font-semibold rounded-lg border-gray-300 dark:border-gray-600 cursor-not-allowed">
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Time of Open Seminar</label>
                        <input type="text" value="{{ $pts1->seminar_time ?? 'N/A' }}" readonly class="w-full bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 font-semibold rounded-lg border-gray-300 dark:border-gray-600 cursor-not-allowed">
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Venue of Open Seminar</label>
                        <input type="text" value="{{ $pts1->seminar_venue ?? 'N/A' }}" readonly class="w-full bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 font-semibold rounded-lg border-gray-300 dark:border-gray-600 cursor-not-allowed">
                    </div>

                    <div class="md:col-span-2">
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Online Meeting Link</label>
                        <input type="text" value="{{ $pts1->meeting_link ?? '' }}" readonly class="w-full bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 font-semibold rounded-lg border-gray-300 dark:border-gray-600 cursor-not-allowed">
                    </div>
                </div>
            </div>

            <!-- Section 3: Institute Norms & Criteria Verification (Read-Only) -->
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 space-y-6">
                <h3 class="text-lg font-bold text-gray-900 dark:text-white border-b border-gray-100 dark:border-gray-700 pb-2">
                    5. Previous Authority Recommendations & Audit Trail
                </h3>
                ...
                <div class="space-y-4">
                    ...
                    @if($hasAnyCoRecommended)
                        <div class="p-4 bg-blue-50/70 dark:bg-blue-950/40 border-l-4 border-blue-500 rounded-xl space-y-3">
                            <h5 class="text-xs font-bold text-blue-900 dark:text-blue-200 uppercase">Co-Supervisor Recommendations</h5>
                    ...
                    @if($hasAnyPspcRecommended)
                        <div class="p-4 bg-purple-50/70 dark:bg-purple-950/40 border-l-4 border-purple-500 rounded-xl space-y-3">
                            <h5 class="text-xs font-bold text-purple-900 dark:text-purple-200 uppercase">PSPC Committee Recommendations</h5>
                    ...
                    @if($passedDpgc)
                        <div class="p-4 bg-teal-50/70 dark:bg-teal-950/40 border-l-4 border-teal-500 rounded-xl space-y-2 text-xs">
                            <div class="flex justify-between font-bold text-teal-900 dark:text-teal-200">
                                ...
                                <span class="font-bold {{ $pts1->dpgc_recommendation ? 'text-emerald-600' : 'text-red-600' }}">
                            ...
                            @if($pts1->dpgc_student_comment)
                                <div class="text-xs text-gray-700 dark:text-gray-300">
                                    ...
                                    <p class="italic bg-white dark:bg-gray-800 p-2 rounded border border-teal-100 dark:border-teal-900 mt-0.5">
                    ...
                    @if($passedHod)
                        <div class="p-4 bg-amber-50/70 dark:bg-amber-950/40 border-l-4 border-amber-500 rounded-xl space-y-2 text-xs">
                            <div class="flex justify-between font-bold text-amber-900 dark:text-amber-200">
                                ...
                                <span class="font-bold {{ $pts1->hod_recommendation ? 'text-emerald-600' : 'text-red-600' }}">
                            ...
                            @if($pts1->hod_student_comment)
                                <div class="text-xs text-gray-700 dark:text-gray-300">
                                    <strong>Student Comment:</strong>
                                    <p class="italic bg-white dark:bg-gray-800 p-2 rounded border border-amber-100 dark:border-amber-900 mt-0.5">
                    ...
                    @if($passedSectionOfficer)
                        <div class="p-4 bg-rose-50/70 dark:bg-rose-950/40 border-l-4 border-rose-500 rounded-xl space-y-2 text-xs">
                            <div class="flex justify-between font-bold text-rose-900 dark:text-rose-200">
                                ...
                                <span class="font-bold {{ $pts1->section_officer_recommendation ? 'text-emerald-600' : 'text-red-600' }}">
                                    {{ $pts1->section_officer_recommendation ? '✓ Verified & Recommended' : '❌ Not Recommended' }}
                                </span>
                            </div>
                            @if($pts1->section_officer_confidential_remark)
                                <div class="text-xs text-gray-700 dark:text-gray-300 pt-0.5">
                                    <strong>Section Officer Remark:</strong>
                                    <p class="italic bg-white dark:bg-gray-800 p-2 rounded border border-rose-100 dark:border-rose-900 mt-0.5">
                    ...
                    <!-- DOAA Approval -->
                    @if($passedDoaa)
                        <div class="p-4 bg-emerald-50/70 dark:bg-emerald-950/40 border-l-4 border-emerald-500 rounded-xl space-y-2 text-xs">
                            <div class="flex justify-between font-bold text-emerald-900 dark:text-emerald-200">
                                <span>Dean of Academic Affairs (DOAA):</span>
                                <span class="font-bold {{ $pts1->doaa_approval ? 'text-emerald-600' : 'text-red-600' }}">
                            ...
                            @if($pts1->doaa_student_comment)
                                <div class="text-xs text-gray-700 dark:text-gray-300">
                                    <strong>Student Comment:</strong>
                                    <p class="italic bg-white dark:bg-gray-800 p-2 rounded border border-emerald-100 dark:border-emerald-900 mt-0.5">
                ...
                <form action="{{ route('pts1.endorse', $pts1->id) }}" method="POST" class="space-y-8">
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-indigo-100 dark:border-indigo-900/50 p-6 space-y-6">
                        <h3 class="text-lg font-bold text-gray-900 dark:text-white border-b border-indigo-100 dark:border-indigo-900/50 pb-2 flex items-center">
                            6. Endorsement Evaluation & Recommendation
                        </h3>

                        <!-- Item 1: Recommendation Radio Cards -->
                        <div class="space-y-4">
                        ...
                        <!-- Item 2: Student Comments Textarea (For DPGC, HOD, DOAA) -->
