    ...
    <div class="py-6">
        <div class="max-w-5xl mx-auto px-2 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 space-y-6">
                ...
                @php
                    $isMinTimeFulfilled = ($viewPerspective === 'student') 
                        ? (bool)$pts1->min_time_req_fulfilled 
                        : (bool)$pts1->effective_min_time_req_fulfilled;
                    
                    $isSpecialMinTimeApproved = ($viewPerspective === 'student')
                        ? (bool)$pts1->special_approval_min_time
                        : (bool)$pts1->effective_special_approval_min_time;
                @endphp
                ...
                <div class="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg space-y-3">
                    <div class="flex items-center justify-between gap-2 sm:gap-4">
                        ...
                        <span class="px-3 py-1 text-xs font-bold rounded-full whitespace-nowrap shrink-0 {{ $isMinTimeFulfilled ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/60 dark:text-emerald-300' : 'bg-red-100 text-red-800 dark:bg-red-900/60 dark:text-red-300' }}">
                            {{ $isMinTimeFulfilled ? 'Yes' : 'No' }}
                        </span>
                    </div>
                    
                    @if(!$isMinTimeFulfilled)
                        <div class="pt-3 border-t border-gray-200 dark:border-gray-600 space-y-2">
                            <div class="flex items-center justify-between gap-2 sm:gap-4">
                                ...
                                <span class="px-3 py-1 text-xs font-bold rounded-full whitespace-nowrap shrink-0 {{ $isSpecialMinTimeApproved ? 'bg-emerald-100 text-emerald-800' : 'bg-red-100 text-red-800' }}">
                                    {{ $isSpecialMinTimeApproved ? 'Yes' : 'No' }}
                                </span>
                            </div>

                            @php
                                $minTimeDocField = ($viewPerspective === 'student') 
                                    ? ($pts1->min_time_approval_doc_path ? 'min_time_approval_doc_path' : null)
                                    : $pts1->getEffectiveMinTimeApprovalField();
                            @endphp
                ...
                <!-- Publication Norm -->
                @php
                    $isPubNormFulfilled = ($viewPerspective === 'student')
                        ? (bool)$pts1->publication_norm_fulfillment
                        : (bool)$pts1->effective_publication_norm_fulfillment;
                    
                    $isSpecialPubApproved = ($viewPerspective === 'student')
                        ? (bool)$pts1->special_approval_publication
                        : (bool)$pts1->effective_special_approval_publication;
                @endphp
                ...
                <div class="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg space-y-3">
                    <div class="flex items-center justify-between gap-2 sm:gap-4">
                        ...
                        <span class="px-3 py-1 text-xs font-bold rounded-full whitespace-nowrap shrink-0 {{ $isPubNormFulfilled ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/60 dark:text-emerald-300' : 'bg-red-100 text-red-800 dark:bg-red-900/60 dark:text-red-300' }}">
                            {{ $isPubNormFulfilled ? 'Yes' : 'No' }}
                        </span>
                    </div>

                    @if(!$isPubNormFulfilled)
                        <div class="pt-3 border-t border-gray-200 dark:border-gray-600 space-y-2">
                            <div class="flex items-center justify-between gap-2 sm:gap-4">
                                <span class="font-semibold text-gray-900 dark:text-white text-sm flex items-center gap-2">
                                ...
                                <span class="px-3 py-1 text-xs font-bold rounded-full whitespace-nowrap shrink-0 {{ $isSpecialPubApproved ? 'bg-emerald-100 text-emerald-800' : 'bg-red-100 text-red-800' }}">
                                    {{ $isSpecialPubApproved ? 'Yes' : 'No' }}
                                </span>
                            </div>

                            @php
                                $pubDocField = ($viewPerspective === 'student') 
                                    ? ($pts1->publication_approval_doc_path ? 'publication_approval_doc_path' : null)
                                    : $pts1->getEffectivePublicationApprovalField();
                            @endphp
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl border border-gray-200 dark:border-gray-600 space-y-2">
                        ...
                        @php
                            $draftDocField = ($viewPerspective === 'student')
                                ? ($pts1->draft_synopsis_report_doc_path ? 'draft_synopsis_report_doc_path' : null)
                                : $pts1->getEffectiveDraftSynopsisField();
                        @endphp
                        @if($draftDocField)
                    ...
                    <div class="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl border border-gray-200 dark:border-gray-600 space-y-2">
                        ...
                        @php
                            $pubListDocField = ($viewPerspective === 'student')
                                ? ($pts1->publication_list_doc_path ? 'publication_list_doc_path' : null)
                                : $pts1->getEffectivePublicationListField();
                        @endphp
                        @if($pubListDocField)
                            <a href="{{ route('pts.document.serve', ['pts1', $pts1->id, $pubListDocField]) }}" target="_blank" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-lg shadow transition inline-flex items-center">
