<x-app-layout>
        ...
        <div class="flex justify-between items-center">
            <div class="flex items-center space-x-3">
                <h2 class="font-bold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                    {{ __('Reverted PTS-2 Form Details & Remarks') }}
                </h2>
    ...
    <div class="py-6">
        <div class="max-w-5xl mx-auto px-2 sm:px-6 lg:px-8 space-y-6">
            ...
            <!-- Prominent Reversion Information Banner -->
            <div class="p-4 bg-amber-50 dark:bg-amber-950/40 border-l-4 border-amber-500 rounded-xl shadow-sm space-y-2">
                <div class="flex items-center space-x-2 text-amber-800 dark:text-amber-200 font-bold">
                    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path>
                    </svg>
                    <span>Form Reverted by {{ $pts2->getRevertedByRoleLabel() }}</span>
                </div>
                @if($pts2->reversion_comment)
                    <div class="text-xs text-gray-700 dark:text-gray-300">
                        <strong>Reversion Reason / Comment:</strong>
                        <p class="italic bg-white dark:bg-gray-800 p-2.5 rounded-lg border border-amber-200 dark:border-amber-900 mt-1 whitespace-pre-wrap break-words [overflow-wrap:anywhere]">{{ trim($pts2->reversion_comment) }}</p>
                    </div>
                @endif
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 space-y-6">
                ...
                @php
                    ...
                    for ($i = 1; $i <= 10; $i++) {
                        ...
                        if ($pts2->$idCol && (!is_null($pts2->$recCol) || !is_null($pts2->$remCol) || !is_null($pts2->$comCol))) {
                            $hasCoSigs = true;
                            break;
                        }
                    }
                    $hasAnyComment = !is_null($pts2->main_supervisor_recommendation) 
                        || !is_null($pts2->main_supervisor_student_comment)
                        || !is_null($pts2->main_supervisor_confidential_remark)
                        || $hasCoSigs 
                        || !is_null($pts2->academic_office_is_verified)
                        || !is_null($pts2->academic_office_verification_remark)
                        || !is_null($pts2->doaa_approval)
                        || !is_null($pts2->doaa_student_comment)
                        || !is_null($pts2->doaa_confidential_remark);
                @endphp

                @if($hasAnyComment)
                    <div class="space-y-4">
                        <!-- Main Supervisor Evaluation -->
                        @if(!is_null($pts2->main_supervisor_recommendation) || !is_null($pts2->main_supervisor_student_comment) || !is_null($pts2->main_supervisor_confidential_remark))
                        ...
                        @if($hasCoSigs)
                            <div class="p-4 bg-blue-50/70 dark:bg-blue-950/40 border-l-4 border-blue-500 rounded-xl space-y-3">
                                ...
                                @for($i = 1; $i <= 10; $i++)
                                    ...
                                    @endphp
                                    @if($coUser && (!is_null($pts2->$recCol) || !is_null($pts2->$remCol) || !is_null($pts2->$comCol)))
                        ...
                        @if(!is_null($pts2->academic_office_is_verified) || !is_null($pts2->academic_office_verification_remark))
                            @php
                        ...
                        <!-- DOAA Approval -->
                        @if(!is_null($pts2->doaa_approval) || !is_null($pts2->doaa_student_comment) || !is_null($pts2->doaa_confidential_remark))
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 space-y-6">
                ...
                <div class="p-4 bg-amber-50 dark:bg-amber-950/40 border-l-4 border-amber-500 rounded-xl">
                    ...
                    @if($pts2->getReversionComment())
                        <div class="mt-2 text-xs text-gray-700 dark:text-gray-300">
                            <strong>Reversion Comment:</strong>
                            <p class="italic bg-white dark:bg-gray-800 p-3 rounded border border-amber-200 dark:border-amber-900 mt-1 whitespace-pre-wrap break-words [overflow-wrap:anywhere]">{{ trim($pts2->getReversionComment()) }}</p>
                        </div>
                    @else
