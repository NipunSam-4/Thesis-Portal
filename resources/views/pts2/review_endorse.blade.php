        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 space-y-6">
                ...
                <div class="space-y-4">
                    ...
                    @if(($coSupervisor1 && $pts2->co_supervisor_1_endorsement) || ($coSupervisor2 && $pts2->co_supervisor_2_endorsement) || ($coSupervisor3 && $pts2->co_supervisor_3_endorsement))
                        <div class="p-4 bg-blue-50/70 dark:bg-blue-950/40 border-l-4 border-blue-500 rounded-xl space-y-3">
                            ...
                            @if($coSupervisor1 && $pts2->co_supervisor_1_endorsement)
                            ...
                            @if($coSupervisor2 && $pts2->co_supervisor_2_endorsement)
                            ...
                            @if($coSupervisor3 && $pts2->co_supervisor_3_endorsement)
                                <div class="text-xs space-y-1 pt-1 border-t border-blue-100 dark:border-blue-900">
                                    <div class="flex justify-between font-semibold">
                    ...
                    @if(($pspc1 && $pts2->pspc_member_1_endorsement) || ($pspc2 && $pts2->pspc_member_2_endorsement) || ($pspc3 && $pts2->pspc_member_3_endorsement))
                        <div class="p-4 bg-purple-50/70 dark:bg-purple-950/40 border-l-4 border-purple-500 rounded-xl space-y-3">
                            ...
                            @if($pspc1 && $pts2->pspc_member_1_endorsement)
                                <div class="text-xs space-y-1">
                                    <div class="flex justify-between font-semibold">
                            ...
                            @if($pspc2 && $pts2->pspc_member_2_endorsement)
                            ...
                            @if($pspc3 && $pts2->pspc_member_3_endorsement)
                                <div class="text-xs space-y-1 pt-1 border-t border-purple-100 dark:border-purple-900">
                                    <div class="flex justify-between font-semibold">
                    ...
                    @if($pts2->section_officer_endorsement)
                ...
                @php
                    ...
                    if ($pts2->current_stage === 'co_supervisors') {
                        if ($pts2->co_supervisor_1_id === $user->id && !$pts2->co_supervisor_1_endorsement) $showActionForm = true;
                        if ($pts2->co_supervisor_2_id === $user->id && !$pts2->co_supervisor_2_endorsement) $showActionForm = true;
                        if ($pts2->co_supervisor_3_id === $user->id && !$pts2->co_supervisor_3_endorsement) $showActionForm = true;
                    } elseif ($pts2->current_stage === 'pspc_members') {
                        if ($pts2->pspc_member_1_id === $user->id && !$pts2->pspc_member_1_endorsement) $showActionForm = true;
                        if ($pts2->pspc_member_2_id === $user->id && !$pts2->pspc_member_2_endorsement) $showActionForm = true;
                        if ($pts2->pspc_member_3_id === $user->id && !$pts2->pspc_member_3_endorsement) $showActionForm = true;
                    } elseif ($pts2->current_stage === 'dpgc') {
                        if ($user->role === 'dpgc' && !$pts2->dpgc_endorsement) $showActionForm = true;
                    } elseif ($pts2->current_stage === 'hod') {
                        if ($user->role === 'hod' && !$pts2->hod_endorsement) $showActionForm = true;
                    } elseif ($pts2->current_stage === 'section_officer') {
                        if ($user->role === 'section_officer' && !$pts2->section_officer_endorsement) $showActionForm = true;
                    } elseif ($pts2->current_stage === 'doaa') {
                        if (in_array($user->role, ['doaa', 'adoaa', 'senate_chairperson', 'ar_academic']) && !$pts2->doaa_endorsement) $showActionForm = true;
                    }
                @endphp
