@php
    ...
    $actingDoaaUsers = $actingDoaaUsers ?? collect();
@endphp
...
<x-app-layout>
    <div class="py-6" x-data="pts4ExtensionReviewForm()">
        <div class="max-w-5xl mx-auto px-2 sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div class="space-y-1">
                    <h2 class="text-xl font-bold text-gray-900 dark:text-white leading-tight">
                        {{ __('PTS-4 Extension Evaluation Portal') }}
                    </h2>
                </div>
            </div>
            ...
            <!-- Section 4: Action Required (Evaluation & Decision) -->
            @if($extension->current_stage === $userRole)
                <form action="{{ route('pts4_extension.endorse', $extension->id) }}" method="POST" class="space-y-8" @submit="clearDraft()">
                    @csrf
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-indigo-100 dark:border-indigo-900/50 p-6 space-y-6">
                        ...
                        @if($userRole === 'academic_office')
                            <!-- Academic Office Verification Checkbox & Declaration -->
                            <div class="space-y-4">
                                ...
                                <label class="p-4 rounded-xl border-2 border-emerald-500 bg-emerald-50/50 dark:bg-emerald-950/30 flex items-start space-x-3 cursor-pointer">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-700 dark:text-gray-300 mt-1 leading-relaxed">
                                            I have verified all student details, academic records, extension request details, and attached documentation for this PTS-4 Extension submission.
                                        </p>
                                    </div>
                                </label>
                            </div>
                            ...
                            <div class="space-y-2 pt-2">
                                ...
                                <div class="pt-0.5">
                                    <x-snippet-dropdown target="confidentialRemark" form-type="pts4_extension" role="academic_office" comment-type="verification_remark" />
                                </div>
    ...
    <script>
        function pts4ExtensionReviewForm() {
            const userId = @js(auth()->id());
            const thesisId = @js($extension->thesis_id);
            const formId = @js($extension->id);
            const draftKey = 'pts4_extension_review_draft_user_' + userId + '_thesis_' + thesisId + '_form_' + formId;
